...
class ManagedBrowser:
    ...
    def build_browser_flags(config: BrowserConfig) -> List[str]:
        ...
        # proxy support - FIXED for BrightData and other proxy services
        if config.proxy:
            # Handle both simple and authenticated proxy URLs
            proxy_url = config.proxy
            if not proxy_url.startswith('http://') and not proxy_url.startswith('https://'):
                proxy_url = f"http://{proxy_url}"
            flags.append(f"--proxy-server={proxy_url}")
        elif config.proxy_config:
            # Use proxy_config for detailed proxy setup
            proxy_server = config.proxy_config.server
            if not proxy_server.startswith('http://') and not proxy_server.startswith('https://'):
                proxy_server = f"http://{proxy_server}"
            
            # Check if credentials are already in the server URL
            if '@' in proxy_server and config.proxy_config.username and config.proxy_config.password:
                # Server URL already has credentials, use as-is
                flags.append(f"--proxy-server={proxy_server}")
            elif config.proxy_config.username and config.proxy_config.password:
                # Add credentials to server URL
                if '://' in proxy_server:
                    protocol, rest = proxy_server.split('://', 1)
                    proxy_url = f"{protocol}://{config.proxy_config.username}:{config.proxy_config.password}@{rest}"
                else:
                    proxy_url = f"http://{config.proxy_config.username}:{config.proxy_config.password}@{proxy_server}"
                flags.append(f"--proxy-server={proxy_url}")
            else:
...
class BrowserManager:
    ...
    def _build_browser_args(self) -> dict:
        ...
        if self.config.proxy or self.config.proxy_config:
            ...
            if self.config.proxy:
                ...
                # Use proxy_config for detailed setup
                proxy_settings = ProxySettings(
    ...
    async def create_browser_context(self, crawlerRunConfig: CrawlerRunConfig = None):
        ...
        # ENHANCED PROXY PARSING: Handle authenticated proxies properly
        proxy_settings = None
        if self.config.proxy:
            proxy_settings = self._parse_proxy_url(self.config.proxy)
    ...
    def _parse_proxy_url(self, proxy_url: str) -> dict:
        """
        Parse proxy URL and return Playwright-compatible proxy configuration.
        
        Handles URLs like: http://username:password@host:port
        Returns: {"server": "http://host:port", "username": "username", "password": "password"}
        """
        ...
        try:
            ...
            self.logger.warning(f"Failed to parse proxy URL {proxy_url}: {e}, using as-is")
